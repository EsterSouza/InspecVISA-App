-- ============================================================
-- 016_calendar_4h_interval_tz_fix.sql
-- 1. Intervalo minimo de 4 horas entre inicios de inspecoes
--    (na pratica ~2 agendamentos por dia util)
-- 2. Conflito por sobreposicao real (antes vazava o slot seguinte)
-- 3. requested_time/period/date gravados no fuso America/Sao_Paulo
--    (antes gravava em UTC e o portal mostrava hora errada)
-- 4. Lock por tenant+dia para evitar corrida de reservas simultaneas
-- 5. Busca publica de clientes desativada (nao expor carteira)
-- ============================================================

revoke execute on function public.public_search_clients(uuid, text) from anon;
revoke execute on function public.public_search_clients(uuid, text) from authenticated;
...
begin
  ...
  -- Serializa reservas concorrentes do mesmo tenant/dia
