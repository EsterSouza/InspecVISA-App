...
set contacts = jsonb_build_array(jsonb_strip_nulls(jsonb_build_object(
  'name', nullif(responsible_name, ''),
  'phone', nullif(phone, ''),
  'email', nullif(email, '')
)))
where contacts = '[]'::jsonb
  and (nullif(responsible_name, '') is not null or nullif(phone, '') is not null or nullif(email, '') is not null);

update public.client_portal_accounts
set payment_links = jsonb_build_array(jsonb_strip_nulls(jsonb_build_object(
  'label', 'Principal',
  'url', nullif(payment_link, '')
)))
where payment_links = '[]'::jsonb
  and nullif(payment_link, '') is not null;
...
begin
  ...
  if jsonb_typeof(v_links) <> 'array' then
    raise exception 'links de pagamento invalidos';
  end if;
...
revoke execute on function public.admin_set_portal_payment(uuid, text, text, text, date, jsonb) from public;
revoke execute on function public.admin_set_portal_payment(uuid, text, text, text, date, jsonb) from anon;
grant execute on function public.admin_set_portal_payment(uuid, text, text, text, date, jsonb) to authenticated;
