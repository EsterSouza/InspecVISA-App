-- ============================================================
-- client_portal_access_email_calendar
-- - Exposes client state in the client portal overview so the UI can
--   block in-person scheduling for units outside RJ.
-- - Enforces the same RJ-only in-person rule inside the public RPC.
-- ============================================================
...
begin
  ...
  select name, state into v_unit_name, v_client_state
  from public.clients
  ...
  if v_attendance_mode = 'presencial'
     and upper(coalesce(trim(v_client_state), '')) not in ('RJ', 'RIO DE JANEIRO') then
