...
import { USERS, CHECKLIST_ACCESS, duploDeConsulta, duploDeSessao } from '../fixtures';
...
describe('🔑 AuthService', () => {
  ...
  describe('getCurrentTenant()', () => {
    test('retorna TenantInfo completo para usuário autenticado', async () => {
      vi.mocked(supabase.auth.getSession).mockResolvedValue({
        data: { session: duploDeSessao({ id: USERS.admin.id, email: USERS.admin.email }) },
        error: null,
      });

      vi.mocked(supabase.from).mockReturnValue(duploDeConsulta({
    ...
    test('retorna null se usuário não está em nenhum tenant', async () => {
      vi.mocked(supabase.auth.getSession).mockResolvedValue({
        data: { session: duploDeSessao({ id: 'user-sem-tenant', email: 'x@x.com' }) },
        error: null,
      });
  ...
  describe('getUserRole()', () => {
    test('retorna role do usuário autenticado', async () => {
      vi.mocked(supabase.auth.getSession).mockResolvedValue({
        data: { session: duploDeSessao({ id: USERS.consultant.id, email: USERS.consultant.email }) },
        error: null,
      });
