    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <Card role="dialog" aria-modal="true" aria-labelledby="portal-access-title" className="max-h-[90vh] w-full max-w-2xl overflow-y-auto shadow-2xl">
        <CardContent className="p-6">
          <h3 id="portal-access-title" className="mb-1 text-xl font-bold text-gray-900">Acesso do portal</h3>
          ...
          <div className="mb-5 rounded-xl border border-gray-100 bg-gray-50 p-3">
            <p id="scheduling-mode-label" className="text-sm font-bold text-gray-800">Agendamento</p>
                  ...
                  <div key={key} className="rounded-xl border border-gray-100 bg-white p-3">
                    <div className="mb-2 flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p id={`feature-${key}-label`} className="text-sm font-semibold text-gray-900">{label}</p>
                        <p className="text-xs text-gray-500">{hint}</p>
                      </div>
                      {busy === key && <Loader2 className="h-4 w-4 shrink-0 animate-spin text-gray-400" aria-hidden="true" />}
                    </div>
                    ...
                    <div className="mt-2 text-xs text-gray-600">
                      <label htmlFor={`hide-at-${key}`}>
                        Ocultar a partir de <span className="text-gray-500">(fim de contrato — opcional)</span>
                      </label>
                      <input
                        id={`hide-at-${key}`}
                        type="datetime-local"
