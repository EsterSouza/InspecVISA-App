...
const STATE_EVENT_CLASSES: Record<WeekCalendarEventState, string> = {
  ...
  'a-confirmar': 'border-gray-200 border-l-gray-400 border-l-dashed bg-gray-50 text-gray-700',
...
const STATE_DOT_CLASSES: Record<WeekCalendarEventState, string> = {
  ...
  'a-confirmar': 'border-gray-400 bg-gray-50',
  ...
  return (
    <div className={cn('overflow-hidden rounded-2xl border border-gray-200 bg-white', className)}>
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 bg-gray-50 px-4 py-3">
        <p className="font-title text-sm font-bold text-gray-900">{week.periodLabel}</p>
        {(onPrevWeek || onNextWeek) && (
          <div className="flex gap-2">
            <button
              ...
              className="flex h-9 w-9 items-center justify-center rounded-md border border-gray-200 text-gray-600 hover:bg-white disabled:cursor-not-allowed disabled:opacity-40"
            >
              ‹
            </button>
            <button
      ...
      <div className="hidden overflow-x-auto min-[721px]:block">
        <div className="sticky top-0 z-10 grid min-w-[660px] grid-cols-[52px_repeat(5,minmax(0,1fr))] border-b border-gray-200 bg-white">
          <div />
          {week.days.map((day) => (
            <div
              key={`${day.label}-${day.dayNumber}`}
              className={cn('border-l border-gray-200 px-2 py-2 text-center', day.isToday && 'bg-primary-50')}
            >
              <p className="text-[11px] font-semibold text-gray-500">{day.label}</p>
              <p className={cn('mt-0.5 font-title text-lg font-bold tabular-nums', day.isToday ? 'text-primary-800' : 'text-gray-900')}>
                {day.dayNumber}
              </p>
              {day.isToday && (
      ...
      <div className="divide-y divide-gray-100 min-[721px]:hidden">
        {week.days.map((day, dayIndex) => {
          const dayEvents = eventsByDay(dayIndex);
          return (
            <div key={`${day.label}-${day.dayNumber}`}>
              <div className="flex items-baseline gap-2 bg-gray-50 px-4 py-2">
                <h4 className="text-sm font-bold text-gray-900">{day.label} {day.dayNumber}</h4>
                ...
                dayEvents.map((event) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={event.onClick}
                      className="flex w-full items-start gap-3 border-t border-gray-100 px-4 py-3 text-left hover:bg-gray-50"
                    >
                      <span className="w-[72px] shrink-0 text-xs tabular-nums text-gray-500">{formatRange(event)}</span>
                      <span className="min-w-0 flex-1">
                        <p className="truncate text-sm font-semibold text-gray-900">{event.title}</p>
                        {event.subtitle && <p className="truncate text-xs text-gray-500">{event.subtitle}</p>}
                      </span>
      ...
      <div className="flex flex-wrap gap-4 border-t border-gray-200 px-4 py-3 text-xs text-gray-600">
