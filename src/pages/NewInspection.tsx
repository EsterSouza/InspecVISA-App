    ...
    <div className="mx-auto max-w-3xl p-4 sm:p-6 lg:p-8">
      ...
      <div className="mb-8">
        ...
        <div className="mt-6 flex items-center justify-between px-2 text-sm font-medium text-gray-500">
          ...
          <ChevronRight className="h-4 w-4 text-gray-300" />
          <span className={step >= 3 ? 'text-primary-600' : ''}>3. Confirmar</span>
        </div>
        ...
        {step === 2 && selectedClient && (
          <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="grid gap-4">
              {templates.filter(t => t.category === selectedClient.category || t.category === 'alimentos').length === 0 ? (
                 <div className="rounded-xl border border-dashed border-gray-300 p-8 text-center text-gray-500">
                   Nenhum roteiro encontrado para esta categoria. (Em desenvolvimento)
                 </div>
              ) : (
                templates
                  .filter(t => t.category === selectedClient.category) // filter matches client category
                  .map(t => (
                  <Card 
                    key={t.id}
                  ...
                  >
                     <div className="flex justify-between items-center">
                        <div>
                          ...
                          <p className="text-sm text-gray-500 mt-1">{t.sections.length} seções • {t.sections.reduce((acc, s) => acc + s.items.length, 0)} itens de verificação</p>
                        </div>
        ...
        {step === 3 && selectedClient && selectedTemplate && (
           <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
             <div className="rounded-xl bg-gray-50 p-6 border border-gray-200">
               <h2 className="text-lg font-semibold text-gray-900 mb-6">Resumo da Inspeção</h2>
               
               <dl className="space-y-4">
                 <div className="grid grid-cols-3 gap-4 border-b border-gray-200 pb-4">
                   <dt className="text-sm font-medium text-gray-500">Cliente</dt>
                   <dd className="text-sm font-semibold text-gray-900 col-span-2">{selectedClient.name}</dd>
                 </div>
                 <div className="grid grid-cols-3 gap-4 border-b border-gray-200 pb-4">
                   <dt className="text-sm font-medium text-gray-500">Categoria</dt>
                   <dd className="text-sm font-semibold text-gray-900 col-span-2 uppercase">{selectedClient.category}</dd>
                 </div>
                 <div className="grid grid-cols-3 gap-4 border-b border-gray-200 pb-4">
                   <dt className="text-sm font-medium text-gray-500">Roteiro aplicável</dt>
                   <dd className="text-sm font-semibold text-primary-700 col-span-2">{selectedTemplate.name}</dd>
                 </div>
                 <div className="grid grid-cols-3 gap-4">
                   <dt className="text-sm font-medium text-gray-500">Data de início</dt>
                   <dd className="text-sm font-semibold text-gray-900 col-span-2">{new Date().toLocaleDateString('pt-BR')}</dd>
                 </div>
               </dl>
             </div>
