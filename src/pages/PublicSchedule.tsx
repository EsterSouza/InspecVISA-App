  ...
  return (
    <div className="min-h-screen bg-surface">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 pb-16 sm:px-6">
          ...
          <section className="grid gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(360px,0.8fr)]">
            <div className="space-y-4">
              <div className="rounded-xl border border-default bg-surface p-4 shadow-sm">
                <span className="block text-sm font-bold text-navy">Consultora <span className="text-red-600">*</span></span>
              ...
              <div className="rounded-xl border border-default bg-surface p-4 shadow-sm">
                <label htmlFor="duration" className="block text-sm font-bold text-navy">Duração</label>
                <select id="duration" value={durationMinutes} onChange={(event) => { setDurationMinutes(Number(event.target.value)); setSelectedTime(null); }} className="mt-2 min-h-11 w-full rounded-md border border-control bg-surface px-3 text-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-100">
              ...
              <div className="rounded-xl border border-default bg-surface p-3 shadow-sm sm:p-4">
                ...
                {calendarLoading ? <div role="status" className="flex h-52 items-center justify-center gap-2 text-sm text-navy-2"><Loader2 className="h-5 w-5 animate-spin text-primary-700" /> Carregando agenda...</div> : calendarError ? (
                  <div role="alert" className="rounded-md border border-red-100 bg-red-50 p-4 text-sm text-red-700">{
                    calendarError
                  }<button type="button" onClick={() => setCalendarReloadKey((value) => value + 1)} className="mt-3 inline-flex min-h-11 items-center gap-2 rounded-md border border-red-200 bg-surface px-3 text-sm font-semibold text-red-700 hover:bg-red-50"><RefreshCw className="h-4 w-4" /> Tentar novamente</button></div>
            ...
            <aside className="rounded-xl border border-default bg-surface p-5 shadow-sm">
              ...
              <div className="mt-4 space-y-4">
                {portalMode ? <div><label htmlFor="unit" className="text-sm font-medium text-navy-2">Unidade <span className="text-red-600">*</span></label><select id="unit" value={selectedClientId} onChange={(event) => { const id = event.target.value; setSelectedClientId(id); const unit = portalUnits.find((item) => item.client_id === id); if (unit?.city) setMunicipality(unit.city); }} className="mt-1.5 min-h-11 w-full rounded-md border border-control bg-surface px-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary-100"><option value="">Selecione uma unidade...</option>{portalUnits.map((unit) => <option key={unit.client_id} value={unit.client_id} disabled={inspection && unitBlockedInSelectedMonth.has(unit.client_id)}>{unit.client_name}{inspection && unitBlockedInSelectedMonth.has(unit.client_id) ? ' — inspeção já solicitada neste mês' : ''}</option>)}</select>{inspection && selectedUnitBlockedInMonth && <p className="mt-1 text-xs text-amber-700">A cota mensal se aplica somente a inspeções.</p>}</div> : <div><label htmlFor="unit-name" className="text-sm font-medium text-navy-2">Nome da unidade ou empresa <span className="text-red-600">*</span></label><input id="unit-name" value={unitName} onChange={(event) => setUnitName(event.target.value)} className="mt-1.5 min-h-11 w-full rounded-md border border-control px-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary-100" /></div>}
                ...
                {attendanceMode === 'presencial' && <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1"><div><label htmlFor="municipality" className="text-sm font-medium text-navy-2">Município <span className="text-red-600">*</span></label><input id="municipality" list="rio-municipios" value={municipality} onChange={(event) => setMunicipality(event.target.value)} className="mt-1.5 min-h-11 w-full rounded-md border border-control px-3 text-sm" /><datalist id="rio-municipios">{RIO_MUNICIPALITIES.map((city) => <option key={city} value={city} />)}</datalist></div><div><label htmlFor="district" className="text-sm font-medium text-navy-2">Bairro <span className="text-red-600">*</span></label><input id="district" value={district} onChange={(event) => setDistrict(event.target.value)} className="mt-1.5 min-h-11 w-full rounded-md border border-control px-3 text-sm" /></div></div>}
        ...
        {step === 3 && <section className="mx-auto max-w-2xl rounded-xl border border-default bg-surface p-5 shadow-sm">{errorBanner}<div className="space-y-4"><div><label htmlFor="subject" className="text-sm font-medium text-navy-2">Assunto</label><input id="subject" value={subject} onChange={(event) => setSubject(event.target.value)} placeholder="Ex.: Revisão de pendências" className="mt-1.5 min-h-11 w-full rounded-md border border-control px-3 text-sm" /></div><div><label htmlFor="objective" className="text-sm font-medium text-navy-2">Objetivo</label><textarea id="objective" value={objective} onChange={(event) => setObjective(event.target.value)} rows={3} className="mt-1.5 w-full rounded-md border border-control p-3 text-sm" /></div><div><label htmlFor="participants" className="flex items-center gap-1.5 text-sm font-medium text-navy-2"><Users className="h-4 w-4 text-navy-3" />Participantes</label><input id="participants" value={participants} onChange={(event) => setParticipants(event.target.value)} placeholder="Separe os nomes por vírgula" className="mt-1.5 min-h-11 w-full rounded-md border border-control px-3 text-sm" /></div><div><label htmlFor="responsible" className="text-sm font-medium text-navy-2">Responsável {!portalMode && <span className="text-red-600">*</span>}</label><input id="responsible" value={responsibleName} onChange={(event) => setResponsibleName(event.target.value)} className="mt-1.5 min-h-11 w-full rounded-md border border-control px-3 text-sm" /></div><div className="grid gap-3 sm:grid-cols-2"><div><label htmlFor="phone" className="flex items-center gap-1.5 text-sm font-medium text-navy-2"><Phone className="h-4 w-4 text-navy-3" />WhatsApp {!portalMode && <span className="text-red-600">*</span>}</label><input id="phone" type="tel" value={phone} onChange={(event) => setPhone(event.target.value)} className="mt-1.5 min-h-11 w-full rounded-md border border-control px-3 text-sm" /></div>{portalMode ? <div className="rounded-md border border-primary-100 bg-primary-50 p-3 text-xs text-primary-900"><strong>E-mail de confirmação</strong><p className="mt-1">Será usado exclusivamente o e-mail cadastrado da unidade selecionada.</p></div> : <div><label htmlFor="email" className="text-sm font-medium text-navy-2">E-mail</label><input id="email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} className="mt-1.5 min-h-11 w-full rounded-md border border-control px-3 text-sm" /></div>}</div><div><label htmlFor="notes" className="flex items-center gap-1.5 text-sm font-medium text-navy-2"><FileText className="h-4 w-4 text-navy-3" />Observações</label><textarea id="notes" value={notes} onChange={(event) => setNotes(event.target.value)} rows={3} className="mt-1.5 w-full rounded-md border border-control p-3 text-sm" /></div></div><div className="mt-6 flex flex-wrap gap-3"><button type="button" onClick={() => setStep(2)} className="inline-flex min-h-11 items-center gap-2 rounded-md border border-default px-4 text-sm font-semibold text-navy-2 hover:bg-surface-hover"><ChevronLeft className="h-4 w-4" />Voltar</button><button type="button" onClick={nextFromDetails} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-primary-700 px-4 text-sm font-semibold text-white hover:bg-primary-800">Revisar solicitação <ChevronRight className="h-4 w-4" /></button></div></section>}
